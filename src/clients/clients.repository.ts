...
export class ClientsRepository {
  ...
  async getAllClients(page: number, limit: number, filters?: ClientFilters) {
    ...
    return {
      clients: clients.map(
        ({ idExternalPassword, paymentMethod, ...clientWithoutPassword }) =>
  ...
  async getClientProfile(id: string) {
    const client = await this.clientsRepository.findOne({
      where: { clientId: id },
      relations: {
        // paymentMethod: true, // Descomentar si se implementa la relación
        // subscriptions: true, // Descomentar si se implementa la relación
    ...
    const { idExternalPassword, paymentMethod, ...clientWithoutPassword } =
      client;
